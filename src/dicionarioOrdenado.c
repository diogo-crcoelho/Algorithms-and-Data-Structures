...
void ordenaDicOrdenado(tuplo * vetor, int nElems,int tipo){
   ...
   tuplo aux;
	for(j=nElems-1; j>=1; j--){
		for(i=0; i<j; i++){
			if(compChaves(priTuplo(vetor[i]),priTuplo(vetor[i+1]),tipo)>0){
				aux=vetor[i];
                vetor[i]=vetor[i+1];
                vetor[i+1]=aux;
            }
...
void ordenaDataOrdenado(tuplo * vetor, int nElems){
   int i, j;
   tuplo aux;
	for(j=nElems-1; j>=1; j--){
		for(i=0; i<j; i++){
			if(compdata(priTuplo(vetor[i]),priTuplo(vetor[i+1]))>0){
				aux=vetor[i];
                vetor[i]=vetor[i+1];
                vetor[i+1]=aux;
            }
...
iterador iteradorDataOrdenada(dicOrdenado d)
    {
    	int i;
	// vector com os elementos do dicionário
	void * * aux = (void * *) malloc(sizeof(void *)* d->numElems);
	ordenaDataOrdenado(d->elems,d->numElems);
	for(i=0;i<d->numElems;i++)
		aux[i]=segTuplo(d->elems[i]);
	return criaIterador(aux,d->numElems);
}
